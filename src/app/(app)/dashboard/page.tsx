...
import type { AgentRun, Recommendation, AgentLog } from '@/types'
import { Package, Store, AlertTriangle, Bell, Clock, Calendar, ChevronRight, TrendingDown, TrendingUp, Minus, Loader2 } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  if (!data) {
    return (
      <div>
        <Topbar title="Dashboard" subtitle="AI-powered competitor price monitoring overview" />
        <div className="p-6 text-center text-gray-400 text-sm">Failed to load dashboard data.</div>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total SKUs" value={data.total_skus} subtitle="Monitored products" icon={Package} iconColor="text-indigo-600" iconBg="bg-indigo-50" />
          <StatCard title="Competitors Tracked" value={data.active_competitors} subtitle="Active sites" icon={Store} iconColor="text-violet-600" iconBg="bg-violet-50" />
          <StatCard title="Price Mismatches" value={data.price_mismatches} subtitle="Found in last run" icon={AlertTriangle} iconColor="text-red-500" iconBg="bg-red-50" />
          <StatCard title="Alerts Sent" value={data.alerts_sent} subtitle="From last run" icon={Bell} iconColor="text-amber-600" iconBg="bg-amber-50" />
        </div>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent className="p-0">
              {data.recent_runs.length === 0 ? (
                <div className="py-10 text-center text-sm text-gray-400">No runs yet — trigger a run from the Monitoring page.</div>
              ) : (
            ...
            <Card>
              <CardContent className="py-4">
                {lastRun ? (
                  ...
                  <div className="py-4 text-center text-sm text-gray-400">No runs yet</div>
                )}
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">Configure in Monitoring</p>
                  </div>
                </div>
                <Link href="/monitoring" className="mt-3 block">
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent className="p-0">
              {data.recent_recommendations.length === 0 ? (
                <div className="py-10 text-center text-sm text-gray-400">No recommendations yet — run the agent first.</div>
              ) : (
                <div className="divide-y divide-gray-100">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-[10px] text-gray-400">
                            {Array.isArray(rec.competitor_data) ? rec.competitor_data.length : 0} competitors
                          </span>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                  ...
                  {data.recent_activity.map((log, i) => (
                    <div key={log.id} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${getLogLevelColor(log.level)}`}>
                          {getLogLevelIcon(log.level)}
                        </div>
                        {i < data.recent_activity.length - 1 && <div className="w-px flex-1 bg-gray-100 mt-1" />}
                      </div>
